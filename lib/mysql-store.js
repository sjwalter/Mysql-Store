...
MysqlStore.prototype.init = function(fn) {
    ...
    this.client.query('CREATE DATABASE `' + this.database + '`;', function(err) {
        ...
        self.client.query('USE `' + self.database + '`;', function(err) {
            ...
            self.client.query('CREATE TABLE IF NOT EXISTS ' + self.table +
                '(sid varchar(64) DEFAULT NULL, sess LONGTEXT DEFAULT NULL);',
                function(err) {
                    if(err) {
                        throw err;
                    }
...
MysqlStore.prototype.get = function(sid, fn) {
    var self = this;
    sys.log('SELECT sess from `' + self.table + '` where `sid`=`' + sid + '`;');
    self.client.query('SELECT sess from `' + self.table + '` where `sid`=\'' + sid + '\';',
        function(err, results) {
            ...
            if(!results || !results[0]) {
                throw new Error('Could not get session with sid: ' + sid);
            }
...
MysqlStore.prototype.set = function(sid, sess, fn) {
    ...
    sys.log('INSERT INTO `' + self.table + '` ' + 'SET sid = ?, sess = ?');
    self.client.query('INSERT INTO `' + self.table + '` ' +
        'SET sid = ?, sess = ?', [sid, sess], 
        function(err) {
            if(err) {
                fn && fn(err);
            }
...
MysqlStore.prototype.destroy = function(sid, fn) {
    this.client.query('DELETE FROM `' + this.table + '` WHERE sid = ?',
        [sid], function() {
            fn();
        }
    );
...
MysqlStore.prototype.length = function(fn) {
    this.client.query('SELECT COUNT(sid) AS count FROM `' + this.table + '`;', 
        function(err, results) {
            if(!results || !results[0]) {
                throw new Error(err);
            }
...
MysqlStore.prototype.clear = function(fn) {
    this.client.query('DELETE FROM `' + this.table +'`;', function() {
        fn();
    });
};
